...
class SawyerPlateSlideSideV2Policy(Policy):
    ...
    def _desired_pos(o_d):
        ...
        if np.linalg.norm(pos_curr[:2] - pos_puck[:2]) > 0.2:
            print('mode 1 dist',np.linalg.norm(pos_curr[:2] - pos_puck[:2]) )
            return pos_puck + np.array([.0, .0, .1])
        elif abs(pos_curr[2] - pos_puck[2]) > 0.04:
            print('mode 2 mode to puck', )
            return pos_puck
        elif pos_curr[0] > -.2:
            print('mode 3', np.array([pos_curr[0] - .1, .6, pos_curr[2]]))
            return np.array([pos_curr[0] - .1, .6, pos_curr[2]])
        else:
            # return pos_puck + np.array([-.1, .0, .0])
            print('mode 4', o_d['goal_pos'] )
            return o_d['goal_pos']